    ...
    public function register(Request $request)
    {
        if($request->isMethod('post')){
            ...
            if($usersCount > 0) {
                ...
                //     $message->to($email)->subject('Registration with Romania Feng Shui Store');
                // });
    ...
    public function confirmAccount($email)
    {
        ...
        if($userCount > 0) {
            ...
            if($userDetails->status == 1) {
                return redirect('login-register')->with('flash_message_success', 'Your email account has been already confirmed! You can login now.');
            }else{
